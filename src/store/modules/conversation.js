...
import store from '..'
import { titleNotify } from '@/utils/index-im'
...
const conversationModules = {
  state: {
    ...
    conversationList: [], // 腾讯的会话列表
    conversationListFlag: false, // 是否有会话
    newConversationList: [], // 自身服务器的会话列表
    callingInfo: {
      memberList: [],
      type: 'C2C' // C2C，GROUP
    },
  ...
  getters: {
    ...
    imgUrlList: state => {
      return state.currentMessageList
        ...
        .map(message => message.payload.imageInfoArray[2].url)  // 原图太慢   预览压缩图
    }
  },
  mutations: {
    /**
     ...
     * @param {Conversation[]} conversationList
     */
      updateConversationListFlag(state, conversationListFlag) {
        state.conversationListFlag = conversationListFlag
      },
     ...
     * @param {Conversation[]} conversationList
     */
    updateNewConversationList(state, conversationList) {
      state.newConversationList = conversationList
    },
    /**
     * 重置当前会话
  ...
  actions: {
    ...
    getMessageList(context, conversationID) {
      const { nextReqMessageID, currentMessageList } = context.state
      const url = process.env.VUE_APP_BASE_API + '/cloud-im/message/queryList'
      request({
        url,
        method:"post",
        data:{
          lastMsgId: nextReqMessageID || '', // 最后一条数据的id
          limitSize: 15,
        ...
        if (arr.length > 0) {
          arr.forEach(item => {
            // senderRole为，表示用户端发出
            item.flow = item.senderRole === '' ? 'in' : 'out'
          })
          // 最后一条
          const last = arr[0]
          // 更新messageID，续拉时要用到
          context.state.nextReqMessageID = last.id // 最后一条数据的id
          context.state.isCompleted = false
          // 更新当前消息列表，从头部插入
          context.state.currentMessageList = [...result.data, ...currentMessageList]
          filterCallingMessage(context.state.currentMessageList)
        } else {
    ...
    checkoutConversation(context, conversationID) {
      ...
      if (context.state.currentConversation.conversationID && store.state.user.duty === '4' && store.state.userIm.isSDKReady) {
      ...
      // 2.待切换的会话也进行已读上报
      if(store.state.user.duty === '4' && store.state.userIm.isSDKReady) {
        tim.setMessageRead({ conversationID })
      }
      // 3. 获取会话信息
      // 注意：这里修改成不读取会话的资料（包括c2c和group类型的，）
      // 3.1 更新当前会话
      // 先要从会话列表中得到当前会话的结构体信息
      let currentCoversation = {}
      context.state.newConversationList.forEach((item) => {
       
        if (item.conversationID === conversationID) {
          currentCoversation = item
        }
      })
     
      context.commit('updateCurrentConversation', currentCoversation)
      // 3.2 获取消息列表
