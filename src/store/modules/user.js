...
const imOnline = localStorage.getItem('imOnline') ? parseInt(localStorage.getItem('imOnline'), 10) : 1
  ...
  login({ commit }, userInfo) {
    ...
    return new Promise((resolve, reject) => {
      login({ userId: username.trim(), password: password }).then(response => {
        ...
        this.state.permission.routes = deepClone([...constantRoutes, ...asyncRoutes])
