  ...
  methods: {
    ...
    async getInfo() {
      const result = await this.$requestPost(
        `/cloud-baseinfo/patient/get?id=${this.sensitiveInfoInit?.data?.id}`
      );
      if (result && result.code === 200) {
        if (this.sensitiveInfoInit.title == "修改手机号") {
          this.ruleForm.inputValue = result.data?.mobile;
        } else if (this.sensitiveInfoInit.title == "修改身份证号") {
    ...
    updateInfo() {
      this.$refs["ruleForm"].validate(async (valid) => {
        if (valid) {
          const params = {
            id: this.sensitiveInfoInit?.data?.id,
            fieldName:
              this.sensitiveInfoInit.title == "修改手机号"
                ? "mobile"
                : "idCard",
            fieldValue: this.ruleForm.inputValue,
          };
          const result = await this.$requestPost(
