  ...
  <div class="container">
    ...
    <div class="chat-wrapper">
      <div class="chat-wrapper-box">
        <div class="chat-wrapper-box-left">
          <side-bar />
        </div>
        <div class="chat-wrapper-box-right">
          <current-conversation @showSchedule="showSchedule"/>
        </div>
      </div>
      <div class="edit-patient-box" v-if="editVisible">
        <patient-edit :editVisible="editVisible" @closePatientEdit="closePatientEdit" />
      </div>

      <div class="edit-patient-box" v-if="editVisibleSchedule">
        <schedule-edit :type="type" :patientId="patientId" :editVisibleSchedule="editVisibleSchedule" @close="closePatientSchedule" />
      </div>
...
import PatientEdit from '@/views/patient/patient-edit'
import ScheduleEdit from '@/views/patient/patient-schedule'
...
export default {
  ...
  mounted() {
    // 初始化监听器
    // this.initListener()
    this.$bus.$on('patient_edit', (val) => {
      this.editVisible = val
    })

    this.$bus.$on('patient_schedule', (val) => {
      this.showSchedule(val)
    })
  },
  created() {

    // this.userID = 'devSystemUser@47'
    // 自动登录
  ...
  methods: {
    ...
    showSchedule({id, type}){
      this.type = type;
      this.patientId = id;
      this.editVisibleSchedule = true
    },
    // refreshData(){
    //   // this.$bus.$emit('refreshData')
    //   this.closePatientSchedule(true)
    // },
    closePatientSchedule() {
      this.editVisibleSchedule = false
    },
