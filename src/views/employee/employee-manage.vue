      ...
      <el-table v-loading="listLoading" class="pa-table" :data="tableData" :height="tableHeight">
        ...
        <el-table-column label="手机号" min-width="180px">
          <template slot-scope="{ row }">
            <template v-if="row.phone">
              <span>{{ row.phone }}</span>
              <SensitiveInfoShow :id="row.id" type="手机号" :url="sensitiveInfoInitUrl" />
        ...
        <el-table-column label="是否启用">
          <template slot-scope="scope">
            <pa-switch :value.sync="scope.row.status" @click="switchClick(scope.row)" />
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
      ...
      if (result && result.code === 200) {
        if (result.data) {
          ...
          this.tableData = result.data.rows.map(item => {
            const deptName = this.deptOptions.find(i => i.value === parseInt(item.deptCode))
            const dutyName = this.dutyOptions.find(i => i.value === item.duty)
            item = {
              ...item,
              status: !!item.status,
    ...
    async switchStatus(row) {
      const params = {}
      const result = await this.$requestPost(
        `/cloud-user/sysUser/isEnabledById?id=${row.id}&status=${row.status ? 0 : 1}`,
        params
      )
      if (result && result.code === 200) {
    ...
    resetClick(row) {
     ...
     // password = 'Service' + password
      ...
      this.$confirm(`<span>是否确定重置密码，密码重置后，新密码为${password}</span>`, '重置密码', {
...
.phone-icons{
 ...
 @media only screen and (max-width: 1645px) and (min-width:1568px) {
  .filter-wrapper{
     ::v-deep .filter-select,.filter-input{
       width: 193px;
     }
  }
}
